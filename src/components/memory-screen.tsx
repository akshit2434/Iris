...
function ActionIcon({ name }: Readonly<{ name: "cancel" | "save" | "edit" | "remove" | "open" }>) {
  if (name === "cancel") return <svg viewBox="0 0 16 16" className="h-3.5 w-3.5" fill="none" stroke="currentColor" strokeLinecap="round" strokeWidth="1.5" aria-hidden="true"><path d="m4 4 8 8m0-8-8 8" /></svg>;
  if (name === "save") return <svg viewBox="0 0 16 16" className="h-3.5 w-3.5" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" aria-hidden="true"><path d="m3.5 8.2 2.8 2.8 6.2-6.2" /></svg>;
  if (name === "edit") return <svg viewBox="0 0 16 16" className="h-3.5 w-3.5" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.35" aria-hidden="true"><path d="m4 11.8 1.1-3.2 5.6-5.6 2.1 2.1-5.6 5.6zM9.6 4l2.1 2.1" /></svg>;
  if (name === "remove") return <svg viewBox="0 0 16 16" className="h-3.5 w-3.5" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.35" aria-hidden="true"><path d="M3.5 5h9M6.3 5V3.5h3.4V5m-5 0 .6 7.5h5.4l.6-7.5M6.8 7.2v3.2m2.4-3.2v3.2" /></svg>;
  return <svg viewBox="0 0 16 16" className="h-3.5 w-3.5" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.35" aria-hidden="true"><path d="M5 3.5h7.5V11M12.2 3.8 4 12" /></svg>;
}
  ...
  return <FluidReveal className="relative mx-auto min-h-[calc(100dvh-4rem)] w-full max-w-5xl px-5 pb-10 pt-12 sm:px-9 sm:pt-20">
    ...
    <div className="relative mx-auto max-w-4xl">
      ...
      <section data-reveal className="glass-surface mt-10 rounded-[28px] p-5 sm:p-6" aria-labelledby="memory-controls-heading">
        <div className="flex flex-wrap items-start justify-between gap-4"><div><h2 id="memory-controls-heading" className="font-semibold tracking-tight text-slate-900">Memory controls</h2><p className="mt-1 max-w-xl text-sm leading-6 text-slate-500">These settings affect only {profileId === "profile-a" ? "this profile" : "this space"}. Turning a layer off keeps existing data but stops Iris from using or writing it.</p></div><button type="button" onClick={() => void clearAllMemory()} disabled={clearing} className="flex h-9 w-9 items-center justify-center rounded-xl text-red-500 transition hover:bg-red-50/80 disabled:opacity-50" aria-label="Remove all memory" title="Remove all memory">{clearing ? <span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-red-300 border-t-red-500" /> : <ActionIcon name="remove" />}</button></div>
...
function MemoryDetailView({ detail, editing, draftContent, saving, onStartEdit, onDraftChange, onCancelEdit, onSave, onForget }: Readonly<{ detail: MemoryDetail; editing: boolean; draftContent: string; saving: boolean; onStartEdit: () => void; onDraftChange: (value: string) => void; onCancelEdit: () => void; onSave: () => void; onForget: () => void }>) {
  return <div>
    ...
    {editing ? <div className="mt-5"><textarea value={draftContent} onChange={(event) => onDraftChange(event.target.value)} rows={5} maxLength={20_000} className="w-full resize-y rounded-2xl border border-white/80 bg-white/55 px-4 py-3 text-[15px] leading-7 text-slate-700" aria-label="Correct memory" /><div className="mt-3 flex flex-wrap justify-end gap-2"><button type="button" onClick={onCancelEdit} disabled={saving} className="flex h-9 w-9 items-center justify-center rounded-xl text-slate-500" aria-label="Cancel correction" title="Cancel correction"><ActionIcon name="cancel" /></button><button type="button" onClick={onSave} disabled={saving || !draftContent.trim()} className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#111827] text-white disabled:opacity-50" aria-label="Save correction" title="Save correction">{saving ? <span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-white/35 border-t-white" /> : <ActionIcon name="save" />}</button></div></div> : <div className="mt-5 text-[15px] text-slate-700"><AssistantMarkdown content={detail.content} live={false} terminal /></div>}
    {!editing && detail.status === "active" ? <div className="mt-5 flex flex-wrap gap-2"><button type="button" onClick={onStartEdit} className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/62 text-slate-600 transition hover:bg-white/85" aria-label="Correct memory" title="Correct memory"><ActionIcon name="edit" /></button><button type="button" onClick={onForget} disabled={saving} className="flex h-9 w-9 items-center justify-center rounded-xl text-red-500 transition hover:bg-red-50/80 disabled:opacity-50" aria-label="Forget memory" title="Forget memory"><ActionIcon name="remove" /></button></div> : null}
    <div className="mt-8 space-y-3"><h2 className="text-xs font-semibold uppercase tracking-[.14em] text-slate-400">Provenance</h2>{detail.revisions.map((revision) => <div key={`${detail.canonicalKey}-${revision.itemRevision}`} className="rounded-2xl bg-white/42 px-4 py-3"><div className="flex flex-wrap justify-between gap-2 text-xs font-medium text-slate-500"><span>Revision {revision.itemRevision} · {revision.mutationKind}</span><span>{formatDate(revision.createdAt)}</span></div>{revision.sources.map((source, index) => <div key={`${revision.itemRevision}-${source.createdAt}-${index}`} className="mt-2 flex flex-wrap items-center justify-between gap-2 text-sm text-slate-600"><span>{source.sourceExcerpt || `Source: ${source.sourceKind}`}</span>{source.action ? <Link className="flex h-8 w-8 items-center justify-center rounded-lg text-[#416fd8] transition hover:bg-white/60" href={buildOpenMessageHref(source.action) ?? "#"} aria-label={source.action.label} title={source.action.label}><ActionIcon name="open" /></Link> : null}</div>)}</div>)}</div>
  </div>;
}
