...
export function toolActionLabel(toolName: string, status: ToolActivity["status"], statusMessage?: string) {
  ...
  if (toolName === "memory_context") return status === "running" ? "Using memory" : status === "failed" ? "Memory unavailable" : "Used memory";
...
export type ToolActivityIconName = "clock" | "chat" | "search" | "memory" | "tools";
...
export function toolActivityIconName(toolName: string): ToolActivityIconName {
  ...
  if (toolName === "memory_context") return "memory";
  return "tools";
}
...
export function toolLabel(toolName: string) {
  ...
  if (toolName === "memory_context") return "Used memory";
...
export function summarizeToolResult(activity: ToolActivity) {
  ...
  if (activity.toolName === "memory_context" && structuredOutput) {
    const items = Array.isArray(structuredOutput.items) ? structuredOutput.items.length : 0;
    const sources = Array.isArray(structuredOutput.sources) ? structuredOutput.sources.length : 0;
    if (items === 0 && sources === 0 && typeof structuredOutput.referenceRevision === "number") return `Reference history r${structuredOutput.referenceRevision}`;
    if (items === 0 && sources === 0) return "No memory details";
    return `${items + sources} ${items + sources === 1 ? "source" : "sources"}`;
  }
...
export function toolDetail(activity: ToolActivity) {
  if (["current_time", "thread_overview", "history_preflight", "search_messages", "read_messages", "memory_context", "memory_list", "memory_read", "memory_search", "memory_patch", "memory_archive", "loop_list", "loop_create", "loop_update", "loop_close", "schedule_check"].includes(activity.toolName)) return null;
