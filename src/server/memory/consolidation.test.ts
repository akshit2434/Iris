...
const job: MemoryConsolidationJob = { id: "00000000-0000-4000-8000-000000000030", profileId: "profile-a", threadId: "00000000-0000-4000-8000-000000000011", sourceRunId: "00000000-0000-4000-8000-000000000012", status: "running", attempts: 1, availableAt: "now", leaseExpiresAt: null, lockedAt: "now", lockedBy: "worker", lastErrorCode: null, lastErrorMessage: null, createdAt: "now", updatedAt: "now", completedAt: null };
...
describe("durable structured memory consolidation", () => {
  ...
  it("rejects foreign sources and duplicate canonical keys", () => {
    ...
    expect(() => validateConsolidationProposals({ job, messages, items }, [{ canonicalKey: "x", proposedContent: "x", expectedItemRevision: null, mutationKind: "create", sourceMessageIds: [messages[0].messageId] }, { canonicalKey: "x", proposedContent: "y", expectedItemRevision: null, mutationKind: "create", sourceMessageIds: [messages[0].messageId] }])).toThrow("duplicate");
  });
  it("processes a bounded job with replay-safe proposal identity and no model/network call", async () => {
    ...
    const producer = vi.fn(async () => [{ canonicalKey: "profile.communication", proposedContent: "The user prefers concise answers.", category: "preference", expectedItemRevision: null, mutationKind: "create", sourceMessageIds: [messages[0].messageId], rationale: "Stable preference" }]);
  ...
  it("does not persist proposals that match an active suppression", async () => {
    ...
    const result = await processConsolidationJobs({
      ...
      memoryStore: store,
      consolidator: createInjectedMemoryConsolidator(async () => [{ canonicalKey: "profile.communication", proposedContent: "The user prefers concise answers.", expectedItemRevision: null, mutationKind: "create", sourceMessageIds: [messages[0].messageId] }]),
      workerId: "worker",
    });
