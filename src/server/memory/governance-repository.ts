...
function toProposal(row: Database["public"]["Tables"]["memory_mutation_proposals"]["Row"]): MemoryMutationProposal {
  return {
    ...
    expectedDocumentRevision: row.expected_document_revision,
...
export function createSupabaseMemoryGovernanceStore(database: MemoryDatabase = getDatabase()): MemoryGovernanceStore {
  return {
    async enqueueConsolidationJob(profileId, threadId, sourceRunId) {
      ...
      const { data, error } = await database.rpc("enqueue_memory_consolidation_job", {
      ...
      if (!row) throw new Error("Consolidation enqueue returned no job.");
      return toJob(row);
    },

    async claimConsolidationJobs(workerId, limit = 1, leaseSeconds = 120) {
      const { data, error } = await database.rpc("claim_memory_consolidation_jobs", {
    ...
    async finishConsolidationJob(input) {
      ...
      validateMemoryUuid(input.jobId, "Job ID");
      ...
      if (error) throw error;
      const row = Array.isArray(data) ? data[0] : data;
      if (!row) throw new Error("Consolidation finish returned no job.");
      return toJob(row);
    },
    ...
    async listJobMessages(profileId, threadId, sourceRunId, limit = 10) {
      ...
      validateMemoryUuid(threadId, "Thread ID");
      validateMemoryUuid(sourceRunId, "Run ID");
      const boundedLimit = normalizeMemoryLimit(limit, 10);
      const { data, error } = await database
        .from("messages")
        .select("id, thread_id, profile_id, content")
        .eq("profile_id", profileId)
        .eq("thread_id", threadId)
        .eq("agent_run_id", sourceRunId)
        .eq("role", "user")
      ...
      return (data ?? []).map((row) => ({
        ...
        threadId: row.thread_id,
        content: row.content,
      } satisfies MemoryMessageForIndex));
    },
    ...
    async insertMutationProposal(input) {
      ...
      validateMemoryUuid(input.jobId, "Job ID");
      const { data, error } = await database
        .from("memory_mutation_proposals")
        .insert({
          ...
          proposed_content_markdown: input.proposedContentMarkdown,
        ...
        .select("id, profile_id, thread_id, source_run_id, job_id, proposal_index, idempotency_key, logical_key, proposed_content_markdown, expected_document_revision, mutation_kind, source_message_ids, rationale, status, reason, result_revision_id, created_at, updated_at, applied_at")
        .single();
    ...
    async applyMutationProposal(profileId, jobId, proposalId, workerId) {
      assertMemoryProfileId(profileId);
      validateMemoryUuid(jobId, "Job ID");
      validateMemoryUuid(proposalId, "Proposal ID");
      const { data, error } = await database.rpc("apply_memory_mutation_proposal", {
        p_profile_id: profileId,
        p_job_id: jobId,
      ...
      if (error) throw error;
      const row = Array.isArray(data) ? data[0] : data;
      if (!row) throw new Error("Proposal apply returned no result.");
      return {
        status: row.status,
