...
export type DeterministicMemoryAcceptanceDeps = {
  patchFact: () => Promise<{ status: "applied"; logicalKey: string; profileGlobalRevision: number }>;
  searchExactSource: (query: string) => Promise<Array<{ messageId: string; threadId: string; excerpt: string }>>;
  readCanonicalFact: (logicalKey: string) => Promise<{ logicalKey: string; contentMarkdown: string } | null>;
...
export async function runDeterministicMemoryAcceptance(deps: DeterministicMemoryAcceptanceDeps) {
  ...
  const recalled = await deps.readCanonicalFact(applied.logicalKey);
  ...
  return {
    chatA: { status: applied.status, logicalKey: applied.logicalKey },
