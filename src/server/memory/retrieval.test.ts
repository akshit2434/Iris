...
describe("profile-scoped memory retrieval", () => {
  ...
  it("drops stale canonical provenance whose stored thread no longer matches the source", async () => {
    const memoryStore = store();
    const sourceMessageId = "00000000-0000-4000-8000-000000000041";
    const storedThreadId = "00000000-0000-4000-8000-000000000042";
    const returnedThreadId = "00000000-0000-4000-8000-000000000043";
    memoryStore.getItemAudit = vi.fn(async () => ({
      item: { id: "item", profileId: "profile-a", canonicalKey: "profile.machine", content: "The user owns a LunarBook 14.", itemRevision: 2, category: "personal_fact", valueScope: "single", origin: "inferred", confidence: 0.9, importance: 0.7, sensitivity: "normal", status: "active", validFrom: null, validUntil: null, lastConfirmedAt: null, supersededByItemId: null, createdAt: "2026-08-15T12:00:00.000Z", updatedAt: "2026-08-15T12:00:00.000Z", archivedAt: null, deletedAt: null },
      revisions: [{ id: "revision", profileId: "profile-a", itemId: "item", itemRevision: 2, profileGlobalRevision: 2, canonicalKey: "profile.machine", content: "The user owns a LunarBook 14.", contentHash: "a".repeat(64), category: "personal_fact", valueScope: "single", origin: "inferred", confidence: 0.9, importance: 0.7, sensitivity: "normal", status: "active", validFrom: null, validUntil: null, lastConfirmedAt: null, supersededByItemId: null, mutationKind: "update", idempotencyKey: null, createdAt: "2026-08-15T12:00:00.000Z", sources: [{ id: "source", sourceKind: "message", sourceThreadId: storedThreadId, sourceMessageId, sourceAgentEventId: null, sourceAgentRunId: null, sourceExcerpt: null, metadata: {}, relation: "supports", createdAt: "2026-08-15T12:00:00.000Z" }] }],
    } satisfies MemoryItemAudit));
    memoryStore.readMessageContext = vi.fn(async () => ({
      thread: { id: returnedThreadId, profileId: "profile-a", title: "Wrong chat", createdAt: "2026-08-15T11:00:00.000Z", updatedAt: "2026-08-15T12:00:00.000Z" },
      target: { messageId: sourceMessageId, threadId: returnedThreadId, profileId: "profile-a", role: "user", content: "My laptop is a LunarBook 14.", createdAt: "2026-08-15T12:00:00.000Z" },
      before: [], after: [],
    } satisfies MessageContextWindow));
