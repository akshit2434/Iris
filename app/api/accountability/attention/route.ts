...
import { createProductionAccountabilityRepository } from "@/server/accountability/repository";
...
export async function GET() {
  ...
  try {
    ...
    const snapshot = await createProductionAccountabilityRepository().getAttentionSnapshot(profileId);
    return NextResponse.json(snapshot, { headers: { "Cache-Control": "no-store" } });
  } catch {
    return NextResponse.json({ error: "Could not load accountability attention." }, { status: 500 });
  }
}
